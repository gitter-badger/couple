  ...
  describe('validate', function() {
    ...
    for (_i = 0, _len = testData.length; _i < _len; _i++) {
      ...
      for (_j = 0, _len1 = _ref.length; _j < _len1; _j++) {
        dataMatch = _ref[_j];
        it("should validate " + data.value + " with " + dataMatch, function() {
          var err;
          try {
            return expect(validate(dataMatch)(data.value)).toBe(true);
          } catch (_error) {
            err = _error;
          ...
          if (data !== otherData) {
            _results1.push((function() {
              ...
              for (_l = 0, _len3 = _ref1.length; _l < _len3; _l++) {
                ...
                _results2.push(it("should not validate " + data.value + " with " + dataNoMatch, function() {
                  var err;
                  try {
                    return expect(validate(dataNoMatch)(data.value)).toBe(false);
                  } catch (_error) {
                    err = _error;
